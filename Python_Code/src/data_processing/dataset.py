...
def preload_to_device(df, logger, device):
    ...
    logger.info(
        f"Data loaded to {device} VRAM successfully. Shape: {images_tensor.shape}"
    )
...
def balance_classes_on_gpu(train_images, train_labels, device, logger):
    ...
    if num_neg > num_pos:
        logger.info(
            f"Oversampling train split: POSITIVE ({num_pos}) to match NEGATIVE ({num_neg})"
        )
        ...
        logger.info(
            f"Oversampling train split: NEGATIVE ({num_neg}) to match POSITIVE ({num_pos})"
        )
