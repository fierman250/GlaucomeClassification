...
def run_experiments():
    ...
    for dataset_name in DATASETS_TO_RUN:
        ...
        for model_name in MODELS_TO_RUN:
            ...
            plot_confusion_matrix(metrics['cm'], f"{eval_name} Confusion Matrix", os.path.join(exp_dir, "confusion_matrix.png"))
            plot_roc_curve(metrics['fpr'], metrics['tpr'], metrics['auc'], f"{eval_name} ROC Curve", os.path.join(exp_dir, "roc_curve.png"))
    ...
    pd.DataFrame(all_results).to_excel(os.path.join(RESULTS_DIR, "custom_experiments_summary.xlsx"), index=False)
    logger.info(f"All experiments completed! Summary saved to {RESULTS_DIR}\\custom_experiments_summary.xlsx")
